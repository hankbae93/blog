...
```bash
# 최근 4주간 weekly 파일 존재 여부 확인
YEAR=$(date +%Y)
CURRENT_WEEK=$(date +%V)
echo "=== Weekly Report Health ==="
for i in 0 1 2 3; do
    W=$(printf "%02d" $((CURRENT_WEEK - i)))
    FILE="generated/summaries/weekly/${YEAR}-W${W}.md"
    if [ -f "$FILE" ]; then
        echo "  ${YEAR}-W${W}: ✅ $(wc -l < "$FILE") lines"
    else
        echo "  ${YEAR}-W${W}: ❌ MISSING"
    fi
done

# 최근 3개월 monthly 파일 존재 여부 확인
echo "=== Monthly Report Health ==="
for i in 0 1 2; do
    M=$(date -v-${i}m +%Y-%m)
    FILE="generated/summaries/monthly/${M}.md"
    if [ -f "$FILE" ]; then
        echo "  ${M}: ✅ $(wc -l < "$FILE") lines"
    else
        echo "  ${M}: ❌ MISSING"
    fi
done
...
| 리포트 | 상태 | 비고 |
|--------|------|------|
| W{nn} | ✅ 존재 / ❌ 누락 | 누락 시 catch-up 안내 |
| {YYYY-MM} | ✅ 존재 / ❌ 누락 | 최근 3개월 표시 |

**누락된 리포트가 있으면** catch-up 안내:
```bash
# 주간 catch-up (데일리 파이프라인에서 자동 실행)
bash scripts/run-weekly.sh

# 월간 catch-up
bash scripts/run-monthly.sh
```
